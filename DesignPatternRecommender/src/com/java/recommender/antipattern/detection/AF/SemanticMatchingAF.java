...
public class SemanticMatchingAF extends SemanticMatching {
	...
	public SemanticMatchingAF(int[][] system, String[] classNames,
			BehavioralMetric behavioralMetric) {
			...
			for (int j = 0; j < newSeq2.size(); j++) {
				if (newSeq1.get(i).matches(newSeq2.get(j))) {
					...
					for (int k = 0; k < classNames.length; k++) {
						if (newSeq1.get(i).toLowerCase().matches(classNames[k].toLowerCase())) {
							s = k;
						}
						if (newSeq2.get(j).toLowerCase().matches(classNames[k].toLowerCase())) {
							d = k;
						}
						if (s != -1 && d != -1) {
							break;
